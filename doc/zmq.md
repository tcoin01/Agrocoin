...
The AgroCoin Core daemon can be configured to act as a trusted "border
router", implementing the agrocoin wire protocol and relay, making
...
The ZeroMQ feature in AgroCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building agrocoind:
...
For instance:
    ...
    $ agrocoind -zmqpubhashtx=tcp://127.0.0.1:237563 \
               -zmqpubhashtx=tcp://192.168.1.2:237563 \
               -zmqpubrawtx=ipc:///tmp/agrocoind.tx.raw \
...
These options can also be provided in agrocoin.conf.
...
From the perspective of agrocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into agrocoind directly. Furthermore, no information is
...
using. AgroCoind appends an up-counting sequence number to each

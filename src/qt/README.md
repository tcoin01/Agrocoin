This directory contains the AgroCoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the AgroCoin UI.
...
ClientModel is used by the main application `agrocoingui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a agrocoin:... link from a browser).
...
* UI elements like AgroCoinAmountField, which inherit from QWidget.
* `agrocoinstrings.cpp`: automatically generated
* `agrocoinunits.(h/cpp)`: AGRO / mAGRO / etc handling
* `callback.h`
...
* don't change `local/agrocoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "agrocoin-qt" under "Run", which is where you can also add command line arguments)

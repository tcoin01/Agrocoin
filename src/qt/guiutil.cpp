...
#include <qt/agrocoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AgroCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new AgroCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new AgroCoinAddressCheckValidator(parent));
}

bool parseAgroCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no agrocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("agrocoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!AgroCoinUnits::parse(AgroCoinUnits::AGRO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseAgroCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseAgroCoinURI(uriInstance, out);
}

QString formatAgroCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("agrocoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(AgroCoinUnits::format(AgroCoinUnits::AGRO, info.amount, false, AgroCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openAgroCoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", AGROCOIN_CONF_FILENAME));
    ...
    /* Open agrocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "AgroCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("AgroCoin (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("agrocoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            ...
            optionFile << strprintf("Name=AgroCoin (%s)\n", chain);

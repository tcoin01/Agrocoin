...
const char * const AGROCOIN_CONF_FILENAME = "agrocoin.conf";
const char * const AGROCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/agrocoin/agrocoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //agrocoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", AGROCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\AgroCoin
    // macOS: ~/Library/Application Support/AgroCoin
    // Unix-like: ~/.agrocoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/AgroCoin";
#else
    // Unix-like
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", AGROCOIN_CONF_FILENAME);
